...
import { ShoppingCart, CreditCard, Trash2, Users } from "lucide-react";
...
export default function CartPanel({ onCheckout }: CartPanelProps) {
  const { items, clearCart, getSubtotal, getDiscountAmount, getTaxAmount, getTotal, getItemCount, taxEnabled, customerId, setCustomerId } =
    useCartStore();
  ...
  if (items.length === 0) {
    return (
      <div className="hidden xl:flex flex-col w-[380px] bg-white border-l border-border h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
          <div className="w-16 h-16 rounded-full bg-navy-50 flex items-center justify-center mb-4">
            <ShoppingCart className="w-8 h-8 text-navy-200" />
          </div>
    ...
    <div className="hidden xl:flex flex-col w-[380px] bg-white border-l border-border h-full">
      ...
      <div className="px-5 py-3 border-t border-border bg-background/50 space-y-1.5">
        <div className="flex justify-between text-sm">
          ...
          <span className="font-mono font-medium">{formatRupiah(getSubtotal())}</span>
        </div>
        {getDiscountAmount() > 0 && (
          <div className="flex justify-between text-sm">
        ...
        {taxEnabled && (
          <div className="flex justify-between text-sm">
            ...
            <span className="font-mono font-medium">{formatRupiah(getTaxAmount())}</span>
          </div>
        )}
