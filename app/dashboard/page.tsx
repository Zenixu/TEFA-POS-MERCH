  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      if (json.error || !json.paymentBreakdown) {
        ...
        // Don't set data if error to throw visual error state or keep loading
      } else {
        setData(json);
      }
    } catch (error) {
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <StatsCard
                  ...
                  value={formatRupiah(data.averageTransaction)}
                  icon={TrendingUp}
                  color="navy"
                />
                <StatsCard
                  title="Stok Kritis"
                  value={data.lowStockVariants.length.toString()}
                  subtitle="variant perlu restock"
                  icon={TrendingUp}
                  color={data.lowStockVariants.length > 0 ? "warning" : "emerald"}
                />
              </div>

              {/* Payment Breakdown & Stakeholders */}
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                <StatsCard
                  title="Tunai"
                  value={formatRupiah(data.revenueByMethod.CASH)}
                  subtitle={`${data.paymentBreakdown.CASH} transaksi`}
                  icon={Banknote}
                  color="emerald"
                />
                <StatsCard
                  title="Kartu"
                  value={formatRupiah(data.revenueByMethod.CARD)}
                  subtitle={`${data.paymentBreakdown.CARD} transaksi`}
                  icon={CreditCard}
                  color="navy"
                />
                <StatsCard
                  title="QRIS"
                  value={formatRupiah(data.revenueByMethod.QRIS)}
                  subtitle={`${data.paymentBreakdown.QRIS} transaksi`}
                  icon={QrCode}
                  color="navy"
                />
                <StatsCard
